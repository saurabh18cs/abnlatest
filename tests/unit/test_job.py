...
from job import extract, transform
from unittest.mock import patch
from collections import namedtuple
from job import transform_it_data
...
def test_emp_calls_df(spark):
    ...
    return spark.createDataFrame(test_data, ["id", "area", "calls_made", "calls_successful"])
...
def test_emp_sales_df(spark):
    ...
    return spark.createDataFrame(test_data, ["id", "caller_id", "company", "recipient", "age", "country", "product_sold", "quantity"])

@patch.object(SparkSession, 'read')
def test_extract(mock_read, spark, test_emp_sales_df, test_emp_personal_df, test_emp_calls_df):
    # Mock the read.csv calls to return the mock DataFrames
    mock_read.csv.side_effect = [test_emp_calls_df, test_emp_personal_df, test_emp_sales_df]
...
def test_transform_it_data(spark, test_emp_calls_df, test_emp_personal_df):
    expected_data = [
        (4, 'area4', 84, 65, 'name4', 'address4', 1234567)
    ]
    expected_df = spark.createDataFrame(expected_data, ["id", "area", "calls_made", "calls_successful", "name", "address", "sales_amount"])
